...
def try_daemon_request(
    ...
    # Detect unified askd daemon: use "ask" protocol prefix instead of per-provider prefix
    is_unified = spec.daemon_module.startswith("askd")
    req_prefix = "ask" if is_unified else spec.protocol_prefix
    resp_prefix = req_prefix

    try:
        payload = {
            "type": f"{req_prefix}.request",
            "v": 1,
        ...
        if is_unified:
            payload["provider"] = getattr(spec, "provider_name", "") or spec.protocol_prefix.rstrip("ask") or spec.protocol_prefix
            payload["caller"] = spec.protocol_prefix
        if output_path:
        ...
        with socket.create_connection((host, port), timeout=connect_timeout) as sock:
            ...
            if resp.get("type") != f"{resp_prefix}.response":
                return None
