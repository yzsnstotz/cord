...
# v5.1 fallback: if repo-local session file is missing, reuse workspace-root provider session.
if [ ! -f "$ccb_session_file" ] && [ -n "$rdloop_root" ]; then
  ...
  if [ -f "$root_session_file" ]; then
    echo "[JUDGE][semi-auto/ccb] using workspace-root session file: ${root_session_file}"
