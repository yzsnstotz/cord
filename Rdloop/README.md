...
当任务使用 **semi-auto** 执行模式时，rdloop 不会自行启动 Coder/Judge 进程，而是连接你本机已启动的 CCB（cask/gask）。你需要在终端（建议 tmux）中先启动 CCB，并确认各 AI provider 在线，再在 GUI 中运行任务。完整步骤（CCB 启动、角色配置、注入冲突处理、semi-auto 工作流）见：

**[CCB 协作模式使用指南](../Agent/docs/ccb_collab_guide.md)**

该文档包含：在 tmux 中启动 `ccb codex gemini`、用 `cask ping`/`gask ping` 验证、编辑 `collab_context.md` 配置角色、使用 `ccb_guard.sh --check` 检测与清理 CCB 注入，以及 semi-auto 下的完整操作顺序与 TaskSpec 示例。
